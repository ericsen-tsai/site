...
function BuenCamino({ diaries }: Props) {
  ...
  return (
    <motion.section
      ...
      id={SECTION_NAV_ITEMS.BUEN_CAMINO}
      ref={sectionRefs?.[SECTION_NAV_ITEMS.BUEN_CAMINO]}
      initial={{
        y: animationCompleted?.[SECTION_NAV_ITEMS.BUEN_CAMINO] ? 0 : ANIMATION_Y_OFFSET,
        opacity: 0
      }}
      ...
      viewport={{ once: true }}
      onAnimationComplete={() => {
        onAnimationComplete?.(SECTION_NAV_ITEMS.BUEN_CAMINO);
      }}
    >
      <div className="flex flex-col">
        <div className="mb-8">
          <div className="flex items-center justify-between">
            <h3 className="text-xl font-medium">Camino Latest Shell</h3>
            <Link
              href="/buen-camino"
              className="group relative inline-flex items-center gap-2 rounded-full bg-yellow-500/20 px-4 py-2 text-sm font-medium text-yellow-500 transition-all hover:bg-yellow-500/30"
            >
              <svg
